...
@Component({
    selector: 'm-collection-categories-selector',
...
export class CategoriesSelectorComponent implements OnInit {
    ...
    onSubmit(): void {
        ...
        this.dialogRef.close({
            data: this.viewData.selection.selected,
            filter: {
                'status': this.viewFilter.filterStatus,
                'text': this.searchInput.nativeElement.value
            }
    ...
    private init(): void {
        ...
        Promise.all([
            ...
            let categories = this.getSubCategories(value[0]).filter(x => this.viewData.initialCategories.indexOf(x.Id) === -1);
    ...
    private getSubCategories(categories: CategoryModel[]): CategoryModel[] {
        let subCategories: CategoryModel[] = [];
        for (let cat of categories) {
            if (cat.Childs && cat.Childs.length && cat.Childs.length > 0) {
