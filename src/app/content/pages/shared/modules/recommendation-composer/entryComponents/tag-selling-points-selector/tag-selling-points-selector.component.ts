...
@Component({
    selector: 'm-tag-selling-points-selector',
    templateUrl: './tag-selling-points-selector.component.html',
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class TagSellingPointsSelectorComponent implements OnInit, OnDestroy {
    ...
    viewFilter: any = {
        ...
        filterTag: new FormControl("", [<any>Validators.required]),
    ...
    search(): void {
        this.viewFilter.filterTag.markAsTouched();

        if (this.viewFilter.filterTag.invalid) {
        ...
        Promise.all([
            this._sellingPointService.searchByTag(
                this.viewFilter.filterTag.value,
    ...
    private parseInjectionData(data: any) {
        ...
        this.viewFilter.filterMode.setValue(data.filter['tag-filter-mode'] || "search");

        this.viewFilter.filterTag.setValue(data.filter['tag-selling-point'] || "");
        this.viewFilter.filterDate.setValue(data.filter['tag-date-selling-point'] || [new Date(), new Date()]);

        this.viewFilter.filterTimeRange[0].setValue(data.filter['tag-time-selling-point'] && data.filter['tag-time-selling-point'][0] ? data.filter['tag-time-selling-point'][0] : '00');
        this.viewFilter.filterTimeRange[1].setValue(data.filter['tag-time-selling-point'] && data.filter['tag-time-selling-point'][1] ? data.filter['tag-time-selling-point'][1] : '24');

        this.viewFilter.filterStatus.setValue(data.filter['tag-status-selling-point'] || '');
        this.viewFilter.filterText.setValue(data.filter['tag-text-selling-point'] || '');
    }
    ...
    private bindSubscribes(): void {
        ...
        this._obsers.push(
            this.viewFilter.filterTag.valueChanges.subscribe(() => {
                this.filterChange();
            })
        );
        ...
        this._obsers.push(
            this._data$.subscribe(value => {
                if (value) {
                    ...
                    if (this.viewFilter.filterTag.length > 0) {
                        this.search();
                    }
                }
            })
    ...
    private getFilter(): any {
        return {
            'tag-filter-mode': this.viewFilter.filterMode.value,

            'tag-selling-point': this.viewFilter.filterTag.value,

            'tag-date-selling-point': this.viewFilter.filterDate.value,
            'tag-time-selling-point': [this.viewFilter.filterTimeRange[0].value, this.viewFilter.filterTimeRange[1].value],

            'tag-status-selling-point': this.viewFilter.filterStatus.value,
            'tag-text-selling-point': this.viewFilter.filterText.value,
        };
    }
}
